...
process_text_files <- function(directory, output_file) {
  ...
  for (file in files) {
    tryCatch({
      ...
      if(dim(current_data)[1] > 0){
        ...
        #PAR003_DIAG_B_LGC1.2
        # Split the file name by "_" and extract the rhird part which is the arrangement name
        split_values_arr_name = sapply(strsplit(file_name_no_extension, "_"), function(x) x[3])
        ...
        current_data$Arrangement = split_values_arr_name
        
        colnames(current_data) = c('SNP','Inv_Name','split_LG_name', 'Arrangement')
  ...
  colnames(merged_data) = c('SNP', 'Inversion', 'LG', 'Arrangement', 'Contig', 'Pos')
  write.table(merged_data, file = paste0('Full_WithArrangemensts_',output_file), row.names = FALSE, col.names = TRUE, quote = FALSE)
