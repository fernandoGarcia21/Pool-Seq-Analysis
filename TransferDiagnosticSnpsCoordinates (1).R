...
#translate_snp_coord_strand(10, 'test', 3, '2M4M2I1M1D1M2S1M', '+', 16)
...
file_old_inv_diagnostic_snps = 'Figures/Inversions/Full_ParallelogramDiagnosticSNPContigs.trimmed.txt';
file_sam_inversions = 'Figures/Inversions/Parallelogram.Inversions.trimmed.mappedContigs.cigar.sam';
file_contig_lengths = 'Figures/Inversions/InversionsParallelogram.contigLengths.trimmed.txt';
...
for(i_snp in 1:dim(data_old_inv_diagnostic_snps)[1]){
  ...
  if(dim(tmp_sam_snp)[1] > 0){
    ...
    tmp_index_main_alignment = which.max((tmp_sam_snp$flag & 256) == 0) 
    #Find the alignment with the largest number of M
    if(tmp_index_main_alignment > 0){
      ...
      print(paste('The contig', tmp_snp$Contig, 'has no main alignment, so the we do not translate the coordinates.'))
    }
...
write.table(file='Trimmed.TranslatedSNPsFromParallelogram.txt', data_old_inv_diagnostic_snps, quote = FALSE, row.names = FALSE, col.names = TRUE, sep = '\t')
...
tmp_contig_length = data_contig_lengths[data_contig_lengths$Contig == tmp_snp$Contig, ]$Length
#Exctract the alignment information of the SNP
tmp_sam_snp = df_sam[df_sam$qname == tmp_snp$Contig & df_sam$LG == tmp_snp$LG, ]
tmp_index_main_alignment = which.max((tmp_sam_snp$flag & 256) == 0) 
#Use the main alignment to translate the SNP coordinate
tmp_chosen_alignment = tmp_sam_snp[tmp_index_main_alignment,]
tmp_translated_LG_Super = tmp_chosen_alignment$rname
...
print(as.numeric(translate_snp_coord_strand(contig_pos = tmp_snp$Pos, tmp_chosen_alignment$rname, ref_start, cigar, strand, tmp_contig_length, tmp_snp$SNP, df_extra_info)))


cigar_ops <- unlist(regmatches(cigar, gregexpr("[MIDNSHP=X]", cigar)))
cigar_lens <- as.numeric(unlist(regmatches(cigar, gregexpr("[0-9]+", cigar))))
data.frame(cigar_ops, cigar_lens)
